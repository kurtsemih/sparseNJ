...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', required=True, help=' ds1 | 16SE | 23SB2k | ... ')
    ...
    for seed in range(num_of_seed):
        ...
        for ki in range(initial_n_leaves):
            dist_matrix[ki, ki+1:] = np.abs(data_onehot_initial[ki+1:, :, :] - data_onehot_initial[ki, :, :]).sum(axis=-1).sum(
                axis=-1)
    ...
    print('*******************')
    print('DONE=>>>>> ' + args.dataset) #+ ' seed= ' + str(seed))
